...
<div class="col-12 mb-6">
  <div class="d-flex justify-content-between align-items-center">
    <div>
      <h4 class="mt-6 mb-1 fw-bold">Edit Task</h4>
      <p class="text-muted mb-0">{{ $task->title }}</p>
    </div>
    <a href="{{ route('supervisor.tasks.show', $task->id) }}" class="btn btn-secondary">
    ...
    <div class="card">
      <div class="card-header bg-light">
        <h6 class="mb-0 fw-semibold">
          <i class="ti ti-edit me-2"></i>Edit Task Details
        </h6>
      </div>
      <div class="card-body">
        <form method="POST" action="{{ route('supervisor.tasks.update', $task->id) }}" id="taskForm">
          @csrf
          @method('PUT')
          ...
          <div class="row g-4 mb-4">
            ...
            <div class="col-md-12">
              ...
              <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                        rows="4" placeholder="Describe the task requirements">{{ old('description', $task->description) }}</textarea>
              @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="intern_id" class="form-select @error('intern_id') is-invalid @enderror" required>
                ...
                <option value="{{ $intern->int_id }}" 
                  {{ old('intern_id', $task->intern_id) == $intern->int_id ? 'selected' : '' }}>
            ...
            <div class="col-md-6">
              ...
              <input type="number" name="points" class="form-control @error('points') is-invalid @enderror" 
                     value="{{ old('points', $task->points) }}" min="1" max="100" step="1" required>
              @error('points')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
          ...
          <div class="d-flex gap-3">
            <button type="submit" class="btn btn-primary px-5">
              <i class="ti ti-device-floppy me-2"></i>Update Task
            </button>
            <a href="{{ route('supervisor.tasks.show', $task->id) }}" class="btn btn-secondary px-4">
              Cancel
            </a>
          </div>
        </form>
