          ...
          <form method="GET" action="{{ route('contact-admin') }}" id="filterForm" class="d-flex gap-2">
            ...
            <div class="btn-group" role="group">
              ...
              <div class="dropdown-menu" style="z-index: 1021" aria-labelledby="btnGroupDrop1">
                <a class="dt-button dropdown-item" href="javascript:void(0);" onclick="downloadContactCSV()">
                  <span>
            ...
            <thead class="border-top sticky-top bg-card">
              <tr>
                ...
                {{-- <th data-dt-column="1" rowspan="1" colspan="1" class="dt-select dt-orderable-none" aria-label="">
                  <span class="dt-column-title"></span><span class="dt-column-order"></span><input
                    class="form-check-input" type="checkbox" aria-label="Select all rows">
                </th> --}}
                ...
                {{-- <th data-dt-column="7" rowspan="1" colspan="1" class="dt-orderable-none" aria-label="Join Date">
                  <span class="dt-column-title">Allow</span><span class="dt-column-order"></span>
                </th> --}}
                <th data-dt-column="7" rowspan="1" colspan="1" class="dt-orderable-none" aria-label="Join Date"><span
      ...
      <div class="row mx-3 justify-content-between">
        ...
        <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto">
          <div class="dt-info" aria-live="polite">
            Showing {{ $contact->firstItem() ?? 0 }} to {{ $contact->lastItem() ?? 0 }} of {{ $contact->total() ??
            0 }} entries
          </div>
        </div>
        ...
        <div
          ...
          <div class="dt-paging">
            <nav aria-label="pagination">
              <ul class="pagination">
                {{-- First Page --}}
                <li class="dt-paging-button page-item {{ $contact->onFirstPage() ? 'disabled' : '' }}">
                ...
                {{-- Page Numbers --}}
                @foreach ($contact->getUrlRange(max(1, $contact->currentPage() - 2), min($contact->lastPage(),
                $contact->currentPage() + 2)) as $page => $url)
                <li class="dt-paging-button page-item {{ $page == $contact->currentPage() ? 'active' : '' }}">
                ...
                {{-- Next Page --}}
                <li
                  class="dt-paging-button page-item {{ $contact->currentPage() == $contact->lastPage() ? 'disabled' : '' }}">
...
<script>
  function downloadContactCSV() {
    ...
    const status = statusFilter ? statusFilter.value : 'contact';
    
    let url = "{{ route('contact.interns.export.csv.admin') }}";
