...
<div class="col-12 mb-6">
    <h4 class="mt-6 mb-1">Contact Interns</h4>
</div>
        ...
        <div id="DataTables_Table_0_wrapper" class="dt-container dt-bootstrap5 dt-empty-footer">
            ...
            <div class="row m-3 my-0 justify-content-between">
                ...
                <div class="d-md-flex align-items-center dt-layout-end col-md-auto ms-auto d-flex gap-2 flex-wrap">
                    <form method="GET" action="{{ route('contact-admin') }}" id="filterForm" class="d-flex gap-2">
...
$label = $packageLabels[$pkg] ?? ($intern->package ?: 'Training');
            ...
            <div class="row mx-3 justify-content-between">
                <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto">
                    <div class="dt-info">Showing {{ $contact->firstItem() ?? 0 }} to {{ $contact->lastItem() ?? 0 }} of {{ $contact->total() ?? 0 }} entries</div>
                </div>
                <div class="d-md-flex align-items-center dt-layout-end col-md-auto ms-auto d-flex gap-2 flex-wrap">
                    <div class="dt-paging">
                        <nav aria-label="pagination">
                            <ul class="pagination">
                                <li class="page-item {{ $contact->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $contact->url(1) }}"><i class="icon-base ti tabler-chevrons-left icon-18px"></i></a>
                                </li>
                                <li class="page-item {{ $contact->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $contact->previousPageUrl() }}"><i class="icon-base ti tabler-chevron-left icon-18px"></i></a>
                                </li>
                                @foreach ($contact->getUrlRange(max(1, $contact->currentPage() - 2), min($contact->lastPage(), $contact->currentPage() + 2)) as $page => $url)
                                <li class="page-item {{ $page == $contact->currentPage() ? 'active' : '' }}"><a class="page-link" href="{{ $url }}">{{ $page }}</a></li>
                                @endforeach
                                <li class="page-item {{ $contact->currentPage() == $contact->lastPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $contact->nextPageUrl() }}"><i class="icon-base ti tabler-chevron-right icon-18px"></i></a>
                                </li>
                                <li class="page-item {{ $contact->currentPage() == $contact->lastPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $contact->url($contact->lastPage()) }}"><i class="icon-base ti tabler-chevrons-right icon-18px"></i></a>
                                </li>
                            </ul>
                        </nav>
                    </div>
...
<script>
    ...
    function downloadCSV() {
        window.location.href = "{{ route('contact.interns.export.csv.admin') }}?search=" + document.getElementById('searchInput').value + "&status=contact";
    }
