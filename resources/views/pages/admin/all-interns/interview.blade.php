          ...
          <form method="GET" action="{{ route('interview-admin') }}" id="filterForm" class="d-flex gap-2">
            ...
            <div class="btn-group" role="group">
              ...
              <div class="dropdown-menu" style="z-index: 1021" aria-labelledby="btnGroupDrop1">
                <a class="dt-button dropdown-item" href="javascript:void(0);" onclick="downloadInterviewCSV()">
                  <span>
      ...
      <div class="row mx-3 justify-content-between">
        ...
        <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto">
          <div class="dt-info" aria-live="polite">
            Showing {{ $interview->firstItem() ?? 0 }} to {{ $interview->lastItem() ?? 0 }} of {{ $interview->total() ??
            0 }} entries
          </div>
        </div>
        ...
        <div
          ...
          <div class="dt-paging">
            <nav aria-label="pagination">
              <ul class="pagination">
                {{-- First Page --}}
                <li class="dt-paging-button page-item {{ $interview->onFirstPage() ? 'disabled' : '' }}">
                ...
                {{-- Page Numbers --}}
                @foreach ($interview->getUrlRange(max(1, $interview->currentPage() - 2), min($interview->lastPage(),
                $interview->currentPage() + 2)) as $page => $url)
                <li class="dt-paging-button page-item {{ $page == $interview->currentPage() ? 'active' : '' }}">
                ...
                {{-- Next Page --}}
                <li
                  class="dt-paging-button page-item {{ $interview->currentPage() == $interview->lastPage() ? 'disabled' : '' }}">
...
<script>
  function downloadInterviewCSV() {
    ...
    const status = statusFilter ? statusFilter.value : 'interview';
    
    let url = "{{ route('interview.interns.export.csv.admin') }}";
