              ...
              <tr>
                ...
                {{-- <th data-dt-column="1" rowspan="1" colspan="1" class="dt-select dt-orderable-none" aria-label="">
                  <span class="dt-column-title"></span><span class="dt-column-order"></span><input
                    class="form-check-input" type="checkbox" aria-label="Select all rows"></th> --}}
      ...
      <div class="row mx-3 justify-content-between">
        ...
        <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto">
          <div class="dt-info" aria-live="polite">
            Showing {{ $completed->firstItem() ?? 0 }} to {{ $completed->lastItem() ?? 0 }} of {{ $completed->total() ??
            0 }} entries
          </div>
        </div>
        ...
        <div
          ...
          <div class="dt-paging">
            <nav aria-label="pagination">
              <ul class="pagination">
                {{-- First Page --}}
                <li class="dt-paging-button page-item {{ $completed->onFirstPage() ? 'disabled' : '' }}">
                ...
                {{-- Page Numbers --}}
                @foreach ($completed->getUrlRange(max(1, $completed->currentPage() - 2), min($completed->lastPage(),
                $completed->currentPage() + 2)) as $page => $url)
                <li class="dt-paging-button page-item {{ $page == $completed->currentPage() ? 'active' : '' }}">
                ...
                {{-- Next Page --}}
                <li
                  class="dt-paging-button page-item {{ $completed->currentPage() == $completed->lastPage() ? 'disabled' : '' }}">
...
<script>
  function downloadCompletedCSV() {
    ...
    const status = statusFilter ? statusFilter.value : 'completed';
    
    let url = "{{ route('completed.interns.export.csv.admin') }}";
