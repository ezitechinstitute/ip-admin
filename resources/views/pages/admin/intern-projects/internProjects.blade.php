...
<div class="col-12 mb-6">
  <h4 class="mt-6 mb-1">Intern Projects</h4>
</div>
{{-- Error Messages --}}
@if($errors->any())
    ...
    <div id="DataTables_Table_0_wrapper" class="dt-container dt-bootstrap5 dt-empty-footer">
      <div class="row m-3 my-0 justify-content-between">
        ...
        <div
          ...
          <form method="GET" action="{{ route('intern-projects') }}" id="filterForm" class="d-flex gap-2">
            ...
            <div class="btn-group" role="group">
              ...
              <div class="dropdown-menu" style="z-index: 1021" aria-labelledby="btnGroupDrop1">
                <a class="dt-button dropdown-item" href="javascript:void(0);" onclick="downloadInternProjectsCSV()">
                  <span>
      ...
      <div class="justify-content-between dt-layout-table">
        <div class="d-md-flex justify-content-between align-items-center dt-layout-full table-responsive overflow-auto"
          ...
          <table class="datatables-users table dataTable dtr-column" id="DataTables_Table_0"
            ...
            <thead class="border-top sticky-top bg-card">
              <tr>
                ...
                <th data-dt-column="4" rowspan="1" colspan="1" class="dt-orderable-asc dt-orderable-desc text-nowrap"
                  aria-label="City" tabindex="0"><span class="dt-column-title" role="button">Title</span><span
                    class="dt-column-order"></span></th>
                ...
                <th data-dt-column="5" rowspan="1" colspan="1" class="dt-orderable-none text-nowrap"
                  aria-label="Join Date"><span class="dt-column-title">Points</span><span
                    class="dt-column-order"></span></th>
                <th data-dt-column="6" rowspan="1" colspan="1" class="dt-orderable-none text-nowrap"
                  aria-label="Join Date"><span class="dt-column-title">STATUS</span><span
                    class="dt-column-order"></span></th>
                <th data-dt-column="5" rowspan="1" colspan="1" class="dt-orderable-none text-nowrap"
                  aria-label="Join Date"><span class="dt-column-title">Task</span><span class="dt-column-order"></span>
                </th>
            ...
            <tbody>
              @forelse ($internProjects as $projects)
              <tr class="">
                <td><span class="text-heading text-nowrap">{{$projects->intern->name}}</span></td>
             
                <td><span
                    class="text-truncate d-flex align-items-center text-heading text-nowrap">{{$projects->title}}</span>
                </td>
                <td><span
                    class="text-truncate d-flex align-items-center text-heading text-nowrap">{{$projects->start_date}}</span>
                </td>
                <td><span
                    class="text-truncate d-flex align-items-center text-heading text-nowrap">{{$projects->end_date}}</span>
                </td>
                <td><span
                    class="text-truncate d-flex align-items-center text-heading text-nowrap">{{$projects->duration}}</span>
                </td>
                <td><span
                    class="text-truncate d-flex align-items-center text-heading text-nowrap">{{$projects->days}}</span>
                </td>
                <td><span
                    class="text-truncate d-flex align-items-center text-heading text-nowrap">{{$projects->obt_marks}}/{{$projects->project_marks}}</span>
                </td>
                <td>
                  @php
                ...
                <td><span class="text-heading text-nowrap">{{$projects->tasks_count}}</span></td>
                <td>
                  <div class="d-flex align-items-center">
                    ...
                    <a href="javascript:void(0);"
                      ...
                      data-bs-toggle="modal" data-bs-target="#editInternModal" data-id="{{ $projects->project_id }}"
                      data-name="{{ $projects->title }}"
                      data-status="{{ $projects->pstatus }}">
                      <i class="icon-base ti tabler-edit icon-22px"></i>
                    </a>

{{-- Edit Interne Project - Start --}}
                      ...
                      <div class="modal-dialog modal-md modal-simple modal-dialog-centered">
                        <div class="modal-content p-2">
                          <div class="modal-body">
                            ...
                            <form id="editInternForm" class="row g-3" action="{{route('update.intern.project.admin')}}"
                              method="POST">
                              @csrf
                              ...
                              <div class="col-12 col-md-6 mb-3">
                                ...
                                <input type="text" id="project_title" required name="project_title" class="form-control" />
                              </div>
                              ...
                              <div class="col-12 col-md-6 mb-3">
                                ...
                                <select name="pstatus" required id="pstatus" class="form-select text-capitalize">
      ...
      <div class="row mx-3 justify-content-between">
        ...
        <div class="d-md-flex justify-content-between align-items-center dt-layout-start col-md-auto me-auto">
          <div class="dt-info" aria-live="polite">
            Showing {{ $internProjects->firstItem() ?? 0 }} to {{ $internProjects->lastItem() ?? 0 }} of {{
            $internProjects->total() ??
            0 }} entries
          </div>
        </div>
          ...
          <div class="dt-paging">
            <nav aria-label="pagination">
              <ul class="pagination">
                ...
                <li class="dt-paging-button page-item {{ $internProjects->onFirstPage() ? 'disabled' : '' }}">
                  <a class="page-link" href="{{ $internProjects->url(1) }}" aria-label="First">
                ...
                <li class="dt-paging-button page-item {{ $internProjects->onFirstPage() ? 'disabled' : '' }}">
                  <a class="page-link" href="{{ $internProjects->previousPageUrl() }}" aria-label="Previous">
                    <i class="icon-base ti tabler-chevron-left scaleX-n1-rtl icon-18px"></i>
                  </a>
                </li>


                @foreach ($internProjects->getUrlRange(max(1, $internProjects->currentPage() - 2),
                min($internProjects->lastPage(),
                $internProjects->currentPage() + 2)) as $page => $url)
                <li class="dt-paging-button page-item {{ $page == $internProjects->currentPage() ? 'active' : '' }}">
                ...
                <li
                  class="dt-paging-button page-item {{ $internProjects->currentPage() == $internProjects->lastPage() ? 'disabled' : '' }}">
                  <a class="page-link" href="{{ $internProjects->nextPageUrl() }}" aria-label="Next">
                ...
                <li
                  class="dt-paging-button page-item {{ $internProjects->currentPage() == $internProjects->lastPage() ? 'disabled' : '' }}">
                  <a class="page-link" href="{{ $internProjects->url($internProjects->lastPage()) }}" aria-label="Last">
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (editModal) {
        editModal.addEventListener('show.bs.modal', function (event) {
            ...
            // Extract info from data-* attributes
            const id = button.getAttribute('data-id');
            const title = button.getAttribute('data-name');   // project title
            const status = button.getAttribute('data-status'); // project status

            // Update the modal fields
            editModal.querySelector('#int_id').value = id;
            editModal.querySelector('#project_title').value = title;
            editModal.querySelector('#pstatus').value = status;
        });
    }
});
</script>
...
<script>
  function downloadInternProjectsCSV() {
    const urlParams = new URLSearchParams(window.location.search);
    const search = urlParams.get('search') || '';
    const status = urlParams.get('status') || '';

    let exportUrl = "{{ route('export.intern.projects.csv.admin') }}";
    exportUrl += `?search=${encodeURIComponent(search)}&status=${encodeURIComponent(status)}`;
