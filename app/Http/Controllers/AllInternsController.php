...
/**
 * Get invoices for a specific intern (AJAX endpoint)
 * Used in the admin profile page to display invoices table
 *
...
public function getInternInvoices($id)
{
    try {
        ...
        // Get invoices for this intern by email
        $invoices = \App\Models\Invoice::where('intern_email', $intern->email)
            ->orderBy('created_at', 'desc')
            ->get();
...
/**
 * Get payments for a specific intern (AJAX endpoint)
 * Used in the admin profile page to display payments table
 *
 * @param int $id - Intern ID
 * @return \Illuminate\Http\JsonResponse
 */
public function getInternPayments($id)
{
    try {
        ...
        // Get invoices for this intern to get invoice IDs for payment lookup
        $invoiceIds = \App\Models\Invoice::where('intern_email', $intern->email)
            ->pluck('id');
        
        // Get payments related to these invoices
        $payments = \App\Models\Payment::whereIn('invoice_id', $invoiceIds)
